...
function serializeEnv(values) {
  ...
  for (const key of Object.keys(values)) {
    if (!orderedKeys.includes(key) && values[key] !== undefined && values[key] !== "") {
      lines.push(`${key}=${values[key]}`);
    }
  }
...
async function main() {
  ...
  if (relayerNeedsGeneration) {
    ...
    const nextValues = {
      ...
      VOTING_CONTRACT_ADDRESS: contractAddress || fileValues.VOTING_CONTRACT_ADDRESS || exampleValues.VOTING_CONTRACT_ADDRESS || "",
      CONTRACT_ADDRESS: contractAddress || fileValues.CONTRACT_ADDRESS || exampleValues.CONTRACT_ADDRESS || "",
      API_PORT: fileValues.API_PORT || exampleValues.API_PORT || "3000",
      RELAYER_PORT: fileValues.RELAYER_PORT || exampleValues.RELAYER_PORT || "3001",
      ...
      MAX_RPC_RETRIES: fileValues.MAX_RPC_RETRIES || exampleValues.MAX_RPC_RETRIES || "3",
    };
  ...
  if (!relayerNeedsGeneration && !/^0x[a-fA-F0-9]{64}$/.test(relayerPrivateKey || "")) {
  ...
  try {
    ...
    console.log("- Contract code:", code === "0x" ? "No encontrado" : "Encontrado");
    ...
    if (code === "0x") {
      console.log("⚠️  El contrato no está desplegado en la dirección actual. Ejecuta npm run deploy y actualiza VOTING_CONTRACT_ADDRESS.");
    }
